/**
 * safecoind.js - a binding for node.js which links to libsafecoind.so/dylib.
 * Copyright (c) 2015, BitPay (MIT License)
 *
 * libsafecoind.cc:
 *   A safecoind node.js binding.
 */

#include "libsafecoind.h"
 ...
 * Used only by safecoind functions.
 */
...
NAN_METHOD(SyncPercentage) {
  ...
  // ntr used old version GuessVerificationProgress: double GuessVerificationProgress(CBlockIndex* pindex, bool fSigchecks = true);
  progress = Checkpoints::GuessVerificationProgress(chainActive.Tip()); 
...
NAN_METHOD(GetNextBlockHash) {
  ...
  if (info.Length() < 1 || !info[0]->IsString()) {
    return ThrowError("Usage: safecoind.getNextBlockHash(blockhash)");
  }
 ...
 * IsSynced()
 * safecoind.isSynced()
 * returns a boolean of safecoin is fully synced
 */
NAN_METHOD(IsSynced) {
...
/**
 * StartBitcoind()
 * safecoind.start(callback)
 * Start the safecoind node with AppInit2() on a separate thread.
 */
NAN_METHOD(StartBitcoind) {
  ...
  if (info.Length() >= 2 && info[0]->IsObject() && info[1]->IsFunction()) {
    ...
    return ThrowError(
      "Usage: safecoind.start(callback)");
  }

  //
  // Run safecoind's StartNode() on a separate thread.
  //
...
async_start_node(uv_work_t *req) {
  ...
  if (data->datadir != "") {
    ...
    snprintf(g_data_dir, sizeof(char) * 512, "%s/.safecoind.js", getenv("HOME"));
  }
  g_rpc = (bool)data->rpc;
  ...
  start_node();
  data->result = std::string("safecoind opened.");
}
...
start_node_thread(void) {
  ...
  if (g_data_dir) {
    ...
    if (w >= 10 && w <= argl) {
      ...
      if (set_cooked()) {
        fprintf(stderr, "safecoind.js: Bad -datadir value.\n");
      }
    }
  }
  ...
  try {
    ...
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
      if (set_cooked()) {
        fprintf(stderr,
          "safecoind.js: Specified data directory \"%s\" does not exist.\n",
    ...
    try {
      ...
      if (set_cooked()) {
        fprintf(stderr,
          "safecoind.js: Error reading configuration file: %s\n", e.what());
      }
      shutdown_complete = true;
      _exit(1);
      return;
    ...
    if (!SelectParamsFromCommandLine()) {
      if (set_cooked()) {
        fprintf(stderr,
          "safecoind.js: Invalid combination of -regtest and -testnet.\n");
      }
      shutdown_complete = true;
      _exit(1);
      return;
    ...
    // ntr used old version without scheduler lib
    fRet = AppInit2(threadGroup);

  } catch (std::exception& e) {
     if (set_cooked()) {
       fprintf(stderr, "safecoind.js: AppInit2(): std::exception\n");
     }
  } catch (...) {
    if (set_cooked()) {
      fprintf(stderr, "safecoind.js: AppInit2(): other exception\n");
    }
  }
...
NAN_METHOD(StopBitcoind) {
  ...
  if (info.Length() < 1 || !info[0]->IsFunction()) {
    return ThrowError(
      "Usage: safecoind.stop(callback)");
  }

  Local<Function> callback = Local<Function>::Cast(info[0]);

  //
  // Run safecoind's StartShutdown() on a separate thread.
  //
...
async_stop_node(uv_work_t *req) {
  ...
  data->result = std::string("safecoind shutdown.");
}
...
/**
 * GetBlock()
 * safecoind.getBlock([blockhash,blockheight], callback)
...
NAN_METHOD(GetBlock) {
  ...
  if (info.Length() < 2
      ...
      || !info[1]->IsFunction()) {
    return ThrowError(
      "Usage: safecoind.getBlock([blockhash,blockheight], callback)");
  }
...
/**
 * GetTransaction()
 * safecoind.getTransaction(txid, queryMempool, callback)
 ...
 * safecoind.getTransactionWithBlockInfo(txid, queryMempool, callback)
...
NAN_METHOD(GetTransactionWithBlockInfo) {
  ...
  if (info.Length() < 3
      ...
      || !info[2]->IsFunction()) {
    return ThrowError(
      "Usage: safecoind.getTransactionWithBlockInfo(txid, queryMempool, callback)");
  }
...
NAN_METHOD(IsSpent) {
  if (info.Length() > 2) {
    return ThrowError(
      "Usage: safecoind.isSpent(txid, outputIndex)");
  }
...
NAN_METHOD(GetBlockIndex) {
  ...
  // ntr used old version
  uint256 cw = blockIndex->nChainWork;
 ...
 * safecoind.getMempoolTransactions()
...
/**
 * Init()
 * Initialize the singleton object known as safecoind.
 */
